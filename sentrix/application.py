...
def log_msg(msg: str, level: str = "INFO"):
    configure_logging()
    LOGGER.log(getattr(std_logging, level.upper(), std_logging.INFO), msg)
...
def resolve_storage_path() -> str:
    preferred = SENTRIX_CONFIG.storage_path
    try:
...
def validate_config():
    errors = SENTRIX_CONFIG.validate()
    if errors:
        for e in errors:
            log_msg(f"  ❌ {e}", "ERROR")
        sys.exit(1)
...
class _SentriXTelegramGateway:
    async def send(self, chat_id: int, text: str, **kwargs) -> dict:
        return await tg_send(chat_id, text, **kwargs)

    async def edit(self, chat_id: int, message_id: int, text: str, **kwargs) -> dict:
        return await tg_edit_text(chat_id, message_id, text, **kwargs)

    async def delete(self, chat_id: int, message_id: int) -> dict:
        return await tg_delete(chat_id, message_id)


class _SentriXPermissions:
